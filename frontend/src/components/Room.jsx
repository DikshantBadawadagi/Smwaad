...
const RoomPage = () => {
  ...
  return (
    <div className="min-h-screen bg-[#205afc] flex flex-col items-center justify-center p-6">
      <div className="top-20 w-full max-w-4xl bg-white rounded-lg shadow-md p-8 relative">
        {/* Image C4 */}
        <div className="absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2">
          <img src={C4} alt="C4" className="shadow-lg" />
        </div>
  
        <h1 className="text-4xl font-bold text-center text-white mb-4 mt-8">
          Room Page
        </h1>
        <h4 className="text-lg text-center text-white mb-6">
          {remoteSocketId ? "Connected to the room" : "Waiting for others to join..."}
        </h4>
        ...
        <div className="flex flex-col space-y-4 items-center">
          {myStream && (
            <button
              onClick={sendStreams}
              className="bg-white text-[#205afc] hover:bg-gray-100 font-bold py-2 px-4 rounded-md transition duration-300 ease-in-out"
            >
              Send Stream
            </button>
          )}
          ...
          <div className="flex justify-between w-full">
            {myStream && (
              <div className="w-1/2 pr-2">
                <h1 className="text-2xl font-semibold text-white mb-2">My Stream</h1>
                <ReactPlayer
                  playing
                  muted
                  height="200px"
            ...
            {remoteStream && (
              <div className="w-1/2 pl-2">
                <h1 className="text-2xl font-semibold text-white mb-2">Remote Stream</h1>
                <ReactPlayer
                  playing
                  muted
                  height="200px"
          ...
          <div className="mt-8 w-full">
            ...
            <div className="border p-4 rounded-lg bg-gray-200 shadow-sm min-h-[100px] w-full mb-4">
              <p className="text-[#205afc]">{transcript || "No speech detected."}</p>
            </div>
          ...
          <div className="mt-8 w-full">
            ...
            <div className="border p-4 rounded-lg bg-gray-200 shadow-sm h-[200px] w-full mb-4 overflow-y-auto">
              {messages.map((msg, index) => (
                <p key={index} className="text-[#205afc]">{`${msg.sender}: ${msg.message}`}</p>
              ))}
            </div>
            <div className="flex">
              <input
